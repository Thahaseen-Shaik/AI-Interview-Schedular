...
const interviewTime = new Date(window.INTERVIEW_START_ISO || window.INTERVIEW_TIME);
...
function canAccessMediaDevices() {
    return Boolean(navigator.mediaDevices?.getUserMedia) && (window.isSecureContext || isLocalhostHost());
}

function getMediaAccessHelpText() {
    return 'Camera and microphone require HTTPS or localhost. Open the system-check page from your Render HTTPS URL, a secure Cloudflare tunnel, or localhost instead of a plain HTTP LAN address.';
}
...
async function testMicrophone() {
    if (!canAccessMediaDevices()) {
        ...
        setCheck('system', 'failed', 'Needs HTTPS');
        return;
    }
    try {
...
function runBrowserCheck() {
    ...
    if (online && hasMedia && (window.isSecureContext || isLocalhostHost())) {
        setCheck('system', 'passed', 'Ready');
    } else {
...
if (canAccessMediaDevices()) {
    ...
    setCheck('mic', 'failed', 'Needs HTTPS');
    setCheck('camera', 'failed', 'Needs HTTPS');
    setCheck('system', 'failed', 'Needs HTTPS');
    if (speakerConfirmWrap) speakerConfirmWrap.hidden = true;
    if (micTestBtn) micTestBtn.title = getMediaAccessHelpText();
    if (speakerTestBtn) speakerTestBtn.title = getMediaAccessHelpText();
}
refreshCountdown();
