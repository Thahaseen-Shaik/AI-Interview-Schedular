...
def _normalize_invite_base_url(candidate):
    ...
    if scheme == "http" and hostname and not _is_private_host(hostname):
...
def _resolve_invite_base_url():
    ...
    for candidate in (public_base_url, base_url):
        normalized = _normalize_invite_base_url(candidate)
        if not normalized:
            continue
        if not _is_local_base_url(normalized):
            return normalized
...
def _invite_url_warning(base_url):
    if _is_local_base_url(base_url):
        return (
            "Invite links are using a local address. Camera and microphone work only on localhost or HTTPS. "
            "Set PUBLIC_BASE_URL to a public HTTPS deployment URL for candidates on other devices."
        )
    return ""
...
def _parse_timezone_offset(value):
    if value in (None, ""):
        return None
    try:
        return int(value)
...
def _to_utc_naive(dt_value, timezone_offset_minutes=None):
    if dt_value is None:
        return None
    if dt_value.tzinfo is not None:
        return dt_value.astimezone(timezone.utc).replace(tzinfo=None, microsecond=0)
    if timezone_offset_minutes is not None:
        return (dt_value + timedelta(minutes=timezone_offset_minutes)).replace(microsecond=0)
    return dt_value.replace(microsecond=0)


def _get_interview_start_utc(interview):
    start_value = _parse_datetime_value(interview["interview_time"])
    offset_minutes = _parse_timezone_offset(interview["time_zone_offset_minutes"])
    return _to_utc_naive(start_value, offset_minutes)


def _format_utc_iso(dt_value):
    if not dt_value:
        return ""
...
INTERVIEW_GREETING = (
    "Hello! I am your AI interviewer today. I'll be conducting this session for about 10 to 15 minutes. "
    "Can you please introduce yourself?"
)

INTERVIEW_CORE_QUESTIONS = [
    {
        "id": "background",
        "text": "Thanks for introducing yourself. Could you briefly walk me through your background and the role you are targeting?",
    },
    {
        "id": "recent_project",
        "text": "Tell me about a recent project you worked on and what your specific contribution was.",
    },
    {
        "id": "tools_stack",
        "text": "What tools, frameworks, or technologies did you use on that project, and why did you choose them?",
    },
    {
        "id": "technical_challenge",
        "text": "What was the toughest technical challenge you faced, and how did you solve it?",
    },
    {
        "id": "teamwork",
        "text": "How did you collaborate with teammates or handle feedback while working on that project?",
    },
    {
        "id": "learning",
        "text": "Which technical skill do you want to improve next, and what are you doing to improve it?",
    },
]
...
INTERVIEW_FOLLOWUP_QUESTIONS = [
    {
        "id": "debugging",
        "text": "Can you share one concrete debugging example and explain how you found the root cause?",
    },
    {
        "id": "tradeoffs",
        "text": "What trade-offs did you consider when you made a design or tooling choice?",
    },
    {
        "id": "impact",
        "text": "How did you measure whether that project or feature was successful?",
    },
    {
        "id": "growth",
        "text": "What have you learned recently that made you a stronger developer?",
    },
]

INTERVIEW_CLOSE_MESSAGE = (
    "[CONCLUDE] Thank you for sharing that. We have covered the main points I needed. "
    "Do you have any final questions for me?"
)


def _default_interview_state():
    return {
        "started": False,
        "question_index": 0,
        "asked_question_ids": [],
...
def _load_interview_state(token):
    context = get_interview_context(token)
    if not isinstance(context, dict):
        return _default_interview_state()

    state = _default_interview_state()
    state.update(context)
    state["started"] = bool(state.get("started"))
    try:
        state["question_index"] = max(0, int(state.get("question_index") or 0))
    except (TypeError, ValueError):
        state["question_index"] = 0
    if not isinstance(state.get("asked_question_ids"), list):
        state["asked_question_ids"] = []
    try:
        state["turns"] = max(0, int(state.get("turns") or 0))
    except (TypeError, ValueError):
        state["turns"] = 0
    state["last_ai_message"] = (state.get("last_ai_message") or "").strip()
    state["completed"] = bool(state.get("completed"))
    return state


def _save_interview_state(token, state):
    save_interview_context(token, state)
...
def _polish_question_with_groq(question, history):
    if not GROQ_API_KEY:
        return question

    recent_transcript = _normalize_transcript(history[-8:])
    system_message = (
        "You rewrite interview questions. Keep the question concise, professional, and natural. "
        "Do not repeat prior questions. Return only the rewritten question."
    )
    user_message = (
        f"Question to rewrite:\n{question}\n\n"
        f"Recent interview transcript:\n{recent_transcript}"
    )

    try:
        rewritten = _groq_chat(
            [
        ...
        if rewritten and _question_similarity(rewritten, question) < 0.85:
            return rewritten
    except Exception:
        pass
    return question
...
def _next_interview_question(state, prompt, elapsed_minutes):
    flow = INTERVIEW_CORE_QUESTIONS + INTERVIEW_FOLLOWUP_QUESTIONS
    if state.get("completed"):
        return INTERVIEW_CLOSE_MESSAGE, True, None

    if state["question_index"] >= len(flow):
        state["completed"] = True
        return INTERVIEW_CLOSE_MESSAGE, True, None

    if state["question_index"] < len(INTERVIEW_CORE_QUESTIONS):
        question = INTERVIEW_CORE_QUESTIONS[state["question_index"]]
    else:
        followup_index = state["question_index"] - len(INTERVIEW_CORE_QUESTIONS)
        if followup_index >= len(INTERVIEW_FOLLOWUP_QUESTIONS):
            state["completed"] = True
            return INTERVIEW_CLOSE_MESSAGE, True, None
        question = INTERVIEW_FOLLOWUP_QUESTIONS[followup_index]

    text = question["text"]
    if len((prompt or "").split()) < 6:
        text = f"{text} Could you add one concrete example?"
    return text, False, question["id"]
...
def _schedule_candidate_batch(candidates, interview_time, base_url, time_zone_offset_minutes=None):
    scheduled = schedule_interviews_bulk(
        [
            {**candidate, "time_zone_offset_minutes": time_zone_offset_minutes}
    ...
    email_status = (email_result or {}).get("ok")
    ...
    if isinstance(email_result, dict):
        ...
        email_error_details = email_result.get("details") or ""
        email_provider = email_result.get("provider") or ""

    return scheduled, recipients, email_status, email_error, email_error_details, email_provider
...
def schedule():
    ...
    time_zone_offset_minutes = _parse_timezone_offset(request.form.get("time_zone_offset_minutes"))

    if not name or not email or not interview_time:
        return jsonify({"message": "All fields are required."}), 400
    base_url = _resolve_invite_base_url()
    scheduled, recipients, email_status, email_error, email_error_details, email_provider = _schedule_candidate_batch(
        [
            {
                "candidate_name": name,
                "candidate_email": email,
    ...
    return jsonify(
        {
            ...
            "email_error_details": email_error_details,
...
def schedule_bulk():
    ...
    if email_status == "sent":
        ...
        message = f"Scheduled {len(scheduled)} interviews. Email provider is not configured, so emails were skipped."
    else:
...
def interview_room():
    ...
    interview_time = _get_interview_start_utc(interview)
    slot_end = interview_time + timedelta(minutes=30) if interview_time else None
    is_waiting = bool(interview_time and now < interview_time)
    seconds_until_start = max(0, int((interview_time - now).total_seconds())) if interview_time else 0
    ...
    if mode == "start":
        return render_template(
            ...
            interview_start_iso=_format_utc_iso(interview_time),
            is_waiting=bool(interview_time and now < interview_time),
            seconds_until_start=seconds_until_start,
            entry_mode="start",
            can_start_now=bool(interview_time and now >= interview_time),
            show_waiting=False,
            show_interview=True,
        )
...
def system_check():
    ...
    return render_template(
        ...
        enter_url=url_for("interview_panel", token=token),
    )
...
def interview_start(token):
    ...
    state = _load_interview_state(token)
    if not state.get("started"):
        save_interview_message(token, "ai", INTERVIEW_GREETING)
        state["started"] = True
        state["question_index"] = max(1, state.get("question_index", 0))
        state["last_ai_message"] = INTERVIEW_GREETING
        _save_interview_state(token, state)
...
def chat():
    ...
    history_rows = get_interview_messages(token)
    history = _groq_history_from_rows(history_rows)
    next_question, should_conclude, question_id = _next_interview_question(state, prompt, elapsed_minutes)
    if should_conclude:
        ai_response = next_question
    else:
        ai_response = _polish_question_with_groq(next_question, history)
        if not ai_response:
            ai_response = next_question
        if _question_similarity(ai_response, state.get("last_ai_message", "")) > 0.9:
            ai_response = next_question
    ...
    state["last_ai_message"] = ai_response
    if should_conclude:
        state["completed"] = True
    _save_interview_state(token, state)
    update_interview_started(token)
